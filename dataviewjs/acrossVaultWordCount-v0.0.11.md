...
Notes:
- Sorting is currently done by the Sortable Plugin. Modified Date does not work with Sortable currently.
...
These inline fields are being used as variables in dataview that will save with the patch rather than being reset each time it updates/your vault refreshes.

searchType:: tags
searchTerm:: "2021-06-14"
%%
...
if(searchType === "tags"){
	...
	let mtimes = dv.pages().where(p => formatDate(p.file.mtime) == searchTerm)
...
const getTableContents = () => {
	...
	for(let i=0; i < searchPagePaths.length;i++){
		...
		page.unsafeCachedData != null ? mtime.push(page.stat.mtime) : null;
		page.unsafeCachedData != null ? name.push("[["+page.basename+"]]") : null;
		page.unsafeCachedData != null ? text.push(page.unsafeCachedData) : null;
	};
	...
	for(let i=0; i<text.length;i++){
		...
		output.push(["<strong>Sum</strong>", "", wordCountSum.toString(), charCountSum.toString(), sentenceCountSum.toString() ])
		return output;
	}

	dv.table(["Page", "Modified On", "Words",  "Characters", "Sentences"], getTableContents()
	)
...
/* ========== Send Values To Metadata on Page ========== */
...
const thisFile = dv.pages().where(f => f.file.path == dv.current().file.path).file.path

const {update} = this.app.plugins.plugins["metaedit"].api;
const buttonMaker = () => {
    const btn = this.container.createEl('button', {"text": "Update Inline Fields"});
    const file = this.app.vault.getAbstractFileByPath(thisFile)
	let pn = ["numPages", "modified", "links", "words", "wordsSum", "characters", "charactersSum", "sentences", "sentencesSum"];
	let pv = [pageCount, allMdate, allNames, allWordCount, wordCountSum, allCharCount, charCountSum, allSentenceCount, sentenceCountSum];
	btn.addEventListener('click', async (evt) => {
        evt.preventDefault();
		for(let i = 0; i < pv.length;i++){
			await update(pn[i], pv[i].join(", "), file);
		}
	});
	
    return btn;
}
dv.paragraph("You can update inline fields with the values in the chart by pressing this button:<br><br>")
dv.paragraph(buttonMaker())
dv.paragraph("<br><br>Chart Data as Inline Fields:")
} else {
	dv.paragraph("<strong>!!! Please enable the MetaEdit plugin then reload this page to export chart data. !!!</strong>")
}
